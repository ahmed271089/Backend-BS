...
export class ChatController {
  ...
  sendFriendRequest(@CurrentUser() user: { userId: string }, @Body() dto: SendFriendRequestDto) {
  ...
  acceptFriendRequest(@CurrentUser() user: { userId: string }, @Param('id') id: string) {
    return this.chatService.respondFriendRequest(id, user.userId, true);
  }

  @Patch('friend-requests/:id/reject')
  rejectFriendRequest(@CurrentUser() user: { userId: string }, @Param('id') id: string) {
